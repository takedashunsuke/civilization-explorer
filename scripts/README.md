...
| `run3-001`〜`005` | 講評後の主実験（バッチ **v2**）。同一列 × resilience。005 で初期富の幅を測った |
...
| `LLM_MAX_AGENTS_PER_TURN` | 地域あたりの個人決定人数（`run3-003` は 4、`run3-004` / `run3-005` は 1） |
...
LLM_MAX_AGENTS_PER_TURN=1 ./scripts/run-experiment-batch-v2.sh  # 済: run3-004。005 は同条件＋初期富の幅
