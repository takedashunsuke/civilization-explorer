...
同一ショック列 × 社会構造。計画 42 件は `run3-001`〜`005` で同一。方針: [post-award.md §4.2](../docs/hackathon/post-award.md)
...
| [run3-005](./output/run3-005/summary.md) | 004 ＋初期富の幅 | **39 / 23 / 13 / 13** | **0.008** … 0.003 | 3 | 生存スプレッドは戻る。格差の単調写像ではない |

実験は `run3-005` まで。10 seed やさらに大きいモデルはしない。年齢死とショック帰属死の分離は済。次は UI の同一 ID 比較。
