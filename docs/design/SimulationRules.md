...
| Tick | 1 Turn を進める処理一式。暦年が西暦 3000 年に達したらそれ以上進めない |
...
| `seed` | int | 再現用シード |
| `population_cap` | int | 人口上限（出生で増える上限。初期は列あたり 100〜1000。世界上限は列数×1000 と初期人数の約 1.4 倍の小さい方） |
...
4. Agent を列ごとに `population` 人生成（100〜1000）  
