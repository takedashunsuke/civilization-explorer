...
| 層 | 技術 | 担当 |
|----|------|------|
| 操作・一覧・数値 UI | **PrimeVue**（Nuxt 上） | 初期条件モーダル（対照実験2ステップ / カスタム）、進行ボタン、ヘッダー件数、出来事、状況パネル |
...
| 画面要素 | 想定コンポーネント例 | 機能 |
|----------|----------------------|------|
| 世界パラメータ入力 | `DataTable`, `Dropdown`, `LevelRating`, `InputNumber` | 対照実験: モード選択 → 4 環境。カスタム: 地理・社会・人口 |
...
3. **初期条件（中央モーダル）** — 対照実験はモード選択 → 環境（豊か/乏しい/災害多/標準）。カスタムは地理・社会・人口  
