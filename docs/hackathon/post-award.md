...
> ステータス: 入賞後の引継ぎ（2026-09）／同一ショック列の 1b 再測 run-014（2026-09-20）  
...
Phase A–C 実装、同一ショック列の固定、1b 200 年再測（`run-014`）まで済。  
**次は 1b の 10 seed ではない。** 次の厚みは中〜大モデルの意思決定（Phase D）。詳細は §9–10・§12–13。
...
1b は観測ラベルではなく、集団スタンス経由で **行動件数を動かした**。同一ショック列の下でも社会差は人口・争い・協力に出る。ただし回復／崩壊のラベル分岐にはなっていない。重複実行 `run-012` は削除済み。次の新規 run は `run-015`。

### 次にやること（この順）

1. **Phase D：意思決定モデルを 1b 観測から引き上げる（P3・高）**  
   `run-014` で同一列のもと 1b でも人口・争い・協力が分岐すると分かった。講評が求めるのは中〜大モデルのサンプル住民／集団決定。1b 10 seed を先に増やさない。  
   比較相手は `run-014`（同一列）。`run-011` / `run-013` は列固定前なのでベースラインにしない。
2. **（任意）同一列の stub 200 年 1 seed（中）**  
   ルール層だけの応答を同じ計画で測る。やるなら Phase D の直前、`--stub --reps 1`。やらなくても Phase D は進めてよい。
3. **語り（P0 残り・中）**  
...
| Phase D 1 seed | `run-014` と同じ seed・同じ計画で、保持率か協力比がモデルサイズで説明できる |
...
- [ ] **1b 観測専用から、中〜大規模モデルで集団／サンプル住民の行動決定へ**（§5.4）← いまここ。比較は `run-014`
...
6. **中〜大モデルで集団／サンプル決定（Phase D）**（いまここ。比較は `run-014`）  
...
既定の `./scripts/run-experiment-batch.sh` は **environment** のまま。講評対応は **`--protocol resilience`**。次の新規 run 番号は `run-015`（`run-012` は重複実行のため削除）。
