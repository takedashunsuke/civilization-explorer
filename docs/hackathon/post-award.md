...
> ステータス: 入賞後の引継ぎ（2026-09）／同一列 stub `run3-001` 済。次は Phase D  
...
**次は 1b の 10 seed ではない。** バッチ v1 は残す。講評後の主実験は **バッチ v2**（`run3` 系列）。同一列 stub（`run3-001`）は済。次は Phase D。
...
1b は観測ラベルではなく、集団スタンス経由で **行動件数を動かした**。同一ショック列の下でも社会差は人口・争い・協力に出る。ただし回復／崩壊のラベル分岐にはなっていない。`run-011` は列固定前なので、LLM なしのベースラインになっていない。重複実行 `run-012` は削除済み。講評後の次番号は **`run3-002`**（Phase D 用。v1 既定の `run-015` は使わない）。
...
読み方: stub では共和が最多生存。1b を足すと生存が減り、順位が専制首位に入れ替わる。争いは 0 → 非ゼロ。ルール層と 1b スタンスは、同じ危機でも生存の勝ちを変える。次は Phase D（§5.4）。

### 次にやること（この順）

1. **Phase D：意思決定モデルを 1b 観測から引き上げる（P3・高）** ← いまここ  
   地域スタンスに加え、地域あたり数人だけ `decide_one` / `decide_batch` で協力・争い・移住を上書きする。全員 LLM にはしない。1b は観測用に残す。比較は `run3-001` と `run-014`。出力はバッチ v2 → `run3-002`。
2. **語り（P0 残り・中）**  
...
- [ ] **1b 観測専用から、中〜大規模モデルで集団／サンプル住民の行動決定へ**（§5.4）← いまここ。比較は `run3-001` と `run-014`
...
7. **中〜大モデルで集団／サンプル決定（Phase D）**（いまここ。比較は `run3-001` と `run-014`）  
8. overview / README にメタ安全保障の一文  
