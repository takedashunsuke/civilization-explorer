...
> ステータス: 入賞後の引継ぎ（2026-09）／8B `run3-003` 済・語り・保持率 3 桁・同一 ID 比較済。次は UI または 8B 切り分け（任意）  
...
**次は 1b の 10 seed ではない。** バッチ v1 は残す。講評後の主実験は **バッチ v2**（`run3` 系列）。同一列 stub（`run3-001`）、1b サンプル（`run3-002`）、8B 決定（`run3-003`）、語り、保持率 3 桁、同一 ID 比較まで済。次のコードは UI（デモが要るとき）。実験を足すなら 8B 偏りの切り分け 1 本だけ。
...
1b は観測ラベルではなく、集団スタンス経由で **行動件数を動かした**。同一ショック列の下でも社会差は人口・争い・協力に出る。ただし回復／崩壊のラベル分岐にはなっていない。`run-011` は列固定前なので、LLM なしのベースラインになっていない。重複実行 `run-012` は削除済み。講評後の次番号は **`run3-004`**（v1 既定の `run-015` は使わない）。
...
読み方: stub では共和が最多生存（保持率 0.056）。1b スタンスだけだと生存が減り専制首位、争いが多い。1b サンプル上書き（`run3-002`）は生存をその間に戻す（保持率 0.038…0.019）。8B 決定（`run3-003`）は共同が 10 倍以上に増え、生存は 4 世界とも 10、保持率は **0.002** で差なし、資源は枯渇。サイズは効いたが、**社会差は潰れた**。大きいモデル＝回復、ではない。比較表の保持率は 2026-09-21 から小数 3 桁。同一 ID は初期名簿のみ比較可。1 世界だけ生存は stub 2 / 1b 4 / 8B 15。2 世界以上は 0。
...
4. **（任意）8B の偏りの切り分け** ← 実験を足すならここ  
   決定モデルを戻す／`LLM_MAX_AGENTS_PER_TURN=1` にする／プロンプトで wait を明示する、のどれか 1 本。10 seed やさらに大きいモデルは先にしない
5. **UI（デモが要るとき）**  
   resilience 切替と回復曲線
6. **（並行可）`inequality` を初期富に乗せる**  
   社会差が協力バイアスと再生補正だけにならないようにする
...
* 系列は `run3`。次番号は `run3-004`（v1 の `run-015` は使わない）
...
| ID 予定 | 変えるもの | 目的 | 合格 |
|---------|------------|------|------|
| `run3-004` | 決定モデルを 1b に戻す、または `LLM_MAX_AGENTS_PER_TURN=1`、または wait を明示 | 8B の協力偏りがモデルか人数かプロンプトかを切り分ける | 計画 42 件が 001–003 と同一。生存・共同が 003 から動くこと |

切り分けが終わるまで、1b / 8B の 10 seed と、観測モデルの引き上げはしない。

**コード側で先に直してよいこと（実験なし）**

* ~~比較表の保持率を小数 3 桁にする~~（済。`f3`。run3 再生成済み）
* ~~同一 ID のクロスワールド比較をサマリーの固定枠にする~~（済。初期名簿のみ。run3 は 1 世界生存が stub 2 / 8B 15、2 世界以上は 0）
* （並行可）`inequality` を初期富に乗せる
...
| 高 | 1b は観測＋集団スタンス。住民意思決定の中〜大モデル化は未 | 8B 決定は `run3-003` 済。共同は増えたが社会差は消えた。切り分けが残件 |
...
- [ ] **（任意）8B 偏りの切り分け**（`run3-004`。決定モデルを戻す / 上書き人数=1 / wait 明示のどれか 1 本）
...
12. （任意）8B 偏りの切り分け 1 本（`run3-004`）  
13. UI: resilience 切替と回復曲線（デモが要るとき）  
