...
> ステータス: 入賞後の引継ぎ（2026-09）／`run3-004` 済。`inequality` 初期富はコード済。UI は resilience 切替と回復曲線まで済  
...
講評後は同一ショック × 社会応答でレジリエンスを測った。1b では社会差が出る。8B 決定では共同が突出し差が消える。大きいモデル＝回復ではない。メタ安全保障の一文は README / overview に掲載済み。
...
**次は 1b の 10 seed ではない。** バッチ v1 は残す。講評後の主実験は **バッチ v2**（`run3` 系列）。同一列 stub（`run3-001`）、1b サンプル（`run3-002`）、8B 決定（`run3-003`）、上書き人数=1（`run3-004`）、語り、保持率 3 桁、同一 ID、`inequality` 初期富、UI（resilience 切替・回復曲線）まで済。
...
1b は観測ラベルではなく、集団スタンス経由で **行動件数を動かした**。同一ショック列の下でも社会差は人口・争い・協力に出る。ただし回復／崩壊のラベル分岐にはなっていない。`run-011` は列固定前なので、LLM なしのベースラインになっていない。重複実行 `run-012` は削除済み。実験番号は **`run3-004` まで**。UI は resilience 切替と回復曲線まで済。
...
5. ~~**UI（デモが要るとき）**~~ — 済。プロトコル切替（環境 / 社会）・resilience 4 世界・history 回復曲線・保持率比較表  
   resilience 切替と回復曲線
6. ~~**`inequality` を初期富に乗せる**~~ — コード済。順位は固定、幅だけ変える。`run3-004` には未適用（切り分けを汚さないため）

### 4.2 今後の実証方針

`run3-003` までで、**モデルを大きくすると行動は動くが、協力に寄りすぎて社会差が消える**ことが分かった。以降の実証は「もっと大きなモデル」ではなく、**差が残る条件を一つずつ切り分ける**。

**固定する条件**

* 系列は `run3`。実験は `run3-004` まで。これ以上の seed / モデル上げはしない（v1 の `run-015` は使わない）
...
切り分けは `run3-004`（人数=1）まで済。共同の半分は上書き人数、残りと人口崩壊は 8B 側。これ以上の 10 seed / 観測 8B はしない。UI（resilience 切替・回復曲線）も済。

**コード側で先に直してよいこと（実験なし）**

* ~~比較表の保持率を小数 3 桁にする~~（済。`f3`。run3 再生成済み）
* ~~同一 ID のクロスワールド比較をサマリーの固定枠にする~~（済。初期名簿のみ。run3 は 1 世界生存が stub 2 / 8B 15、2 世界以上は 0）
* ~~`inequality` を初期富に乗せる~~（済。順位固定・幅だけ。004 には未適用）
...
| 高 | 1b は観測＋集団スタンス。住民意思決定の中〜大モデル化は未 | 8B 決定は `run3-003` 済。切り分け `run3-004` も済。共同の半分は人数、残りと人口崩壊は 8B 側 |
| 中 | 年齢死が強く、ラベルが collapsed に寄る | 既知。保持率で読む |
| 中 | `inequality` が初期富に乗らない。stub の社会差は協力バイアスと再生補正が主 | コード済（順位固定・幅だけ変える）。`run3-004` には未適用 |
...
14. ~~`inequality` を初期富に乗せる~~（コード済。004 には未適用）  
...
`run3-003` の読み: 8B は協力を増やしたが、再編成の**差**は消えた。メタ安全保障は「協力が多いこと」ではなく「社会ごとに応答が分かれること」で測る。大きいモデル＝回復、ではない。以降の実験は §4.2 に従う。
...
* 社会ノブの効きにムラ（疫病が交易開放に漏れる）。`inequality` の初期富はコード済（順位固定・幅だけ。004 には未適用）
...
| `inequality` | ロスター生成は 0.5。resilience は `apply_identity_wealth` で幅だけ変える。順位は固定。3 人以上の体制ドリフトは実行時の富分散 |
...
| Phase D | 同一列の 1b で人口・争い・協力が分岐（`run-014` / `run3-002`）。8B 決定（`run3-003`）は共同が突出し社会差が消えた |
