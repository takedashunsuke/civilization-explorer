...
> ステータス: 入賞後の引継ぎ（2026-09）／8B `run3-003` 済・語り・保持率 3 桁済。次は同一 ID 比較（8B 切り分けは任意）  
...
**次は 1b の 10 seed ではない。** バッチ v1 は残す。講評後の主実験は **バッチ v2**（`run3` 系列）。同一列 stub（`run3-001`）、1b サンプル（`run3-002`）、8B 決定（`run3-003`）、語り、保持率 3 桁まで済。次は同一 ID 比較。実験を足すなら 8B 偏りの切り分け 1 本だけ。
...
読み方: stub では共和が最多生存（保持率 0.056）。1b スタンスだけだと生存が減り専制首位、争いが多い。1b サンプル上書き（`run3-002`）は生存をその間に戻す（保持率 0.038…0.019）。8B 決定（`run3-003`）は共同が 10 倍以上に増え、生存は 4 世界とも 10、保持率は **0.002** で差なし、資源は枯渇。サイズは効いたが、**社会差は潰れた**。大きいモデル＝回復、ではない。比較表の保持率は 2026-09-21 から小数 3 桁。
...
- [ ] 同一 ID のクロスワールド生死・台頭比較をサマリーに固定枠で出す ← いまここ
