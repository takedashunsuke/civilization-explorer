...
> ステータス: 入賞後の引継ぎ（2026-09）／8B 決定 `run3-003` 済。共同は増えたが社会差は消えた  
...
**次は 1b の 10 seed ではない。** バッチ v1 は残す。講評後の主実験は **バッチ v2**（`run3` 系列）。同一列 stub（`run3-001`）、1b サンプル（`run3-002`）、8B 決定（`run3-003`）まで済。次は語りと、8B が協力に偏って社会差を潰したことの扱い。
...
読み方: stub では共和が最多生存。1b スタンスだけだと生存が減り専制首位、争いが多い。1b サンプル上書き（`run3-002`）は生存をその間に戻す。8B 決定（`run3-003`）は共同が 10 倍以上に増え、生存は 4 世界とも 10、資源は枯渇。サイズは効いたが、**社会差は潰れた**。大きいモデル＝回復、ではない。

### 次にやること（この順）

1. **語り（P0 残り・中）** ← いまここ  
   README / overview にメタ安全保障の一文（§6）。指標は保持率・協力比・制度破綻。8B の「協力しすぎて差が消える」は失敗ではなく、モデルサイズの効き方として書く。
2. **観測の厚み（P4・中）**  
...
9. overview / README にメタ安全保障の一文。8B 偏りの扱い  
