...
**次は 1b の 10 seed ではない。** 同一ショック列は固定済み。次の厚みは中〜大モデルの意思決定（Phase D）。詳細は §9–10・§12。
...
1. **Phase D：意思決定モデルを 1b 観測から引き上げる（P3・高）**  
   ...
   同一ショック列は 2026-09-19 に固定済み。200 年の再測は Phase D の 1 seed と同時でよい。
2. **語り（P0 残り・中）**  
...
| 優先 | 課題 | 状態 |
|------|------|------|
| 高 | 「同一ショック列」 | 済。`generate_shock_plan` + イベント独立 RNG。パイロットで列一致 |
...
5. ~~同一ショック列の固定~~（`run` パイロットで列一致・ショック数一致を確認）  
6. **中〜大モデルで集団／サンプル決定（Phase D）**（いまここ）  
