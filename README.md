...
入賞時は同一 5,000 人 × 環境差（`run-001`〜`010`）。講評後の主実験は同一ショック列 × 社会構造差（`run3`）。計画 42 件は `run3-001`〜`004` で同一。

| | stub `run3-001` | 1b サンプル `run3-002` | 8B n=4 `run3-003` | 8B n=1 `run3-004` |
|--|--|--|--|--|
| 生存 | civic 278 が最多 | 190 / 182 / 189 / 93 | **4 世界とも 10** | 21 / 23 / 19 / 20 |
| 人口保持率 | **0.056** … 0.025 | 0.038 … 0.019 | **0.002** | 0.004–0.005 |
| 共同 | 0 | 数十 | **318–372** | 136–179 |
| 争い | 0 | 8–47 | ほぼ 0 | civic 5 … fracture **90** |

8B は行動を動かすが回復ではない。上書き人数を 1 に落とすと共同は半減し争いは戻るが、生存の社会差は戻らない。初期名簿で 2 世界以上生き残った人は **0**。生ログ: [result/raw/run3-004/](./result/raw/run3-004/)

## 今後の実証方針

**変えるのは一度に一つ。系列は `run3`。実験は `run3-004` まで。** 詳細は [post-award.md §4.2](./docs/hackathon/post-award.md)。
...
| `inequality` は順位を固定したまま幅だけ変える（コード済） | 全住民を毎ターン LLM |
